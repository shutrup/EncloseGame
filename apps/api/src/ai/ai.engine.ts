import type { AILevel, BoardLayout, Edge, GameState, Zone } from './ai.types';
...
function estimateReplyPressure(edgeId: number, state: GameState, context: AiContext): ReplyPressure {
  ...
  // If we captured and kept the turn, opponent pressure is irrelevant now.
...
function getLocalMoveMetrics(
  edgeId: number,
  state: GameState,
  context: AiContext
): Pick<MoveMetrics, 'captures' | 'createsThird' | 'createsSecond'> {
  let captures = 0;
  let createsThird = 0;
  let createsSecond = 0;
...
function hashState(state: GameState, depth: number, maximizingPlayer: boolean, quiescenceLeft: number): string {
  const occupied = Array.from(state.occupiedEdges)
    .sort((a, b) => a - b)
    .join(',');
...
function getScores(state: GameState): { x: number; o: number } {
  let x = 0;
  let o = 0;
...
function nextPlayer(current: 'x' | 'o'): 'x' | 'o' {
